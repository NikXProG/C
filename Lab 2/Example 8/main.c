...
int main(void){
    ...
    if (result_bool  == NULL){
        free(result_double);
        for (int i=0;i<count_arguments; i++){   
            free(result_double[i]);
        } 
        return MEMORY_ALLOCATION_ERROR;
    
    }

    double_type_has_finite_representation(result_double,result_bool, number_simbols_after_point,2 , count_arguments, (double) 1/5,(double) 0.6875,(double) 1/4 );
    for(int i=0;i<count_arguments;i++){
        if (result_bool[i] == 0){
